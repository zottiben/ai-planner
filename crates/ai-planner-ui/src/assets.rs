...
//! what makes `aip ui` work from a `cargo install`ed binary with no repo, no node and
//! no network, and it is the same reason rule 16 compiles the skill in.
...
/// Whether a real bundle was compiled in, as opposed to the placeholder. `aip doctor`
...
/// `/plan/kanban-ui/slice/PR2` is a 404 - which is the whole point of the drawer
/// having a URL.
...
fn lookup(path: &str) -> Option<(&'static [u8], &'static str)> {
    ...
    // No bundle compiled in. Say so in the browser rather than showing a blank page
    // and letting someone conclude the server is broken.
...
<meta name="color-scheme" content="light dark"><title>ai-planner</title>
<style>
